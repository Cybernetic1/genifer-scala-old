...
class Unify {
  ...
  def unify(t1: Seq[Atom], t2: Seq[Atom], direction: Char, sub: Seq[Atom]): Option[Seq[Seq[Seq[Atom]]]] = {
    ...
    if (a1.isEmpty && a2.isEmpty)
      ...
      if (direction == '+')
        unify(t1, r2, '+', sub. +:(a2.asInstanceOf[Atom]))
      else if (a2.asInstanceOf[Atom].isConst)
        None
      else addSub(sub, unify(t1, r2, '-', Seq(a2.asInstanceOf[Atom])))
    }
    else if (a2.isEmpty) {
      if (direction == '-')
        unify(r1, t2, '-', sub.+:(a1.asInstanceOf[Atom]))
      else if (a1.asInstanceOf[Atom].isConst)
        false
      else addSub(sub, unify(r1, t2, '+', Seq(a1.asInstanceOf[Atom])))
    }
    ...
    direction match {
      ...
      case '0' =>
        if (a1.asInstanceOf[Atom].isConst && a2.asInstanceOf[Atom].isConst && testEq(a1, a2))
            unify(r1, r2, '0', Seq())
        else if (a1.asInstanceOf[Atom].isConst && a2.asInstanceOf[Atom].isConst && testNotEq(a1, a2))
            false
        else if (a1.asInstanceOf[Atom].isVariable && a2.asInstanceOf[Atom].isConst)
            forkSubs(
              addSub(Seq(a1.asInstanceOf[Atom]),
                unify(r1, t2, '0', Seq())),
                // Begin substitution { a2... / a1 }
                unify(r1, r2, '+', Seq(a2, a1)))
        else if (and (const? a1) (variable? a2))
            (fork-subs
              (add-sub [a2]
                (unify t1 r2 0 ()))
...
// -- OUTPUT: a list of compound subs
def addSub(x: Option[Seq[Atom]], y: Option[Seq[Seq[Seq[Atom]]]]): Seq[Seq[Seq[Atom]]] = {
  if (x == None)
    false
  else if (y == None)
    false
